...
(function() {
    ...
    angular.module('osgeoImporter.uploader', [
        ...
        .provider('layerService', function() {
            ...
            this.validateConfigurationOptions = function(layer) {
                ...
                if (layer.configuration_options.always_geogig === true) {
                    ...
                    if (layer.configuration_options.version_control === true) {
                        layer.configuration_options.geoserver_store = {'type': 'geogig'};
                    } else {
        ...
        .controller('ImportController', function ($scope, $uibModal, $log, $q, UploadedData) {
            ...
            function checkForSimilar(layer) {
                return UploadedData.query({
                    file_name: layer ? layer.file_name: ''
                        ...
                        for(var i = 0, ii = data.objects.length; looks_okay && i < ii; i++) {
                            ...
                            if(layer && data.objects[i].layers
        ...
        .controller('WizardController', function ($scope, $http, $modalInstance, layer, layerService, $interval, modalImage, staticUrl, default_config, appendTo, shapefile_link, csv_link) {
            ...
            $scope.defaultPermissions = {'users':{'AnonymousUser':['download_resourcebase', 'view_resourcebase']}};
            var stop;
            ...
            $scope.setDefaults = function() {
                ...
                if ($scope.layer.hasOwnProperty('layer_type') && ($scope.layer.configuration_options.hasOwnProperty('version_control'))) {
                    $scope.layer.configuration_options.version_control = $scope.layer.layer_type !== 'raster';
                }else{
                    $scope.layer.configuration_options.version_control = false;
                }
